...
def get_filter(filt_size=3):
    if(filt_size == 1):
        ...
        a = np.array([[1., 4., 6., 4., 1.],[1., 4., 6., 4., 1.],[1., 4., 6., 4., 1.],[1., 4., 6., 4., 1.],[1., 4., 6., 4., 1.]])
    elif(filt_size == 6):
        a = np.array([[1., 5., 10., 10., 5., 1.],[1., 5., 10., 10., 5., 1.],[1., 5., 10., 10., 5., 1.],[1., 5., 10., 10., 5., 1.],[1., 5., 10., 10., 5., 1.],[1., 5., 10., 10., 5., 1.]])
    elif(filt_size == 7):
        a = np.array([[1., 6., 15., 20., 15., 6., 1.],[1., 6., 15., 20., 15., 6., 1.],[1., 6., 15., 20., 15., 6., 1.],[1., 6., 15., 20., 15., 6., 1.],[1., 6., 15., 20., 15., 6., 1.],[1., 6., 15., 20., 15., 6., 1.],[1., 6., 15., 20., 15., 6., 1.]])
...
class Downsample(nn.Module):
    def __init__(self, channels, pad_type='reflect', filt_size=3, stride=2, pad_off=0):
        ...
        self.pad_sizes = [int(1. * (filt_size - 1) / 2), int(np.ceil(1. * (filt_size - 1) / 2)), int(1. * (filt_size - 1) / 2), int(np.ceil(1. * (filt_size - 1) / 2)),int(np.ceil(1. * (filt_size - 1) / 2)),int(np.ceil(1. * (filt_size - 1) / 2))]
    ...
    def forward(self, inp):
        if(self.filt_size == 1):
            ...
            return F.conv3d(input=self.pad(inp), weight= self.filt.unsqueeze(dim=1), padding=[0],stride=[self.stride], groups=inp.shape[1], dilation=[1])
...
class Upsample(nn.Module):
    ...
    def forward(self, inp):
        ret_val = F.conv_transpose3d(self.pad(inp), weight=self.filt, stride=self.stride, padding=1 + self.pad_size, groups=inp.shape[1])[:, :, 1:, 1:]
        if(self.filt_odd):
            return ret_val
        else:
            return ret_val[:, :, :-1, :-1]
...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal',
    ...
    if netG == 'resnet_9blocks':
        ...
        n_blocks = 5
        net = G_Resnet(input_nc, output_nc, 0, num_downs=2, n_res=n_blocks - 4, ngf=ngf, norm='inst', nl_layer='relu')
    else:
